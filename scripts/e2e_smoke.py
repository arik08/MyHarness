#!/usr/bin/env python3
"""Run real end-to-end MyHarness scenarios against an Anthropic-compatible API."""
...
from myharness.config.paths import get_project_issue_file, get_project_pr_comments_file
...
from myharness.memory import add_memory_entry
...
def _validate_worktree_flow(cwd: Path, final_text: str, tool_names: list[str], started: int, completed: int) -> tuple[bool, str]:
    worktree_path = cwd / ".myharness" / "worktrees" / "smoke-worktree"
...
SCENARIOS: dict[str, Scenario] = {
    "file_io": Scenario(
        name="file_io",
        prompt=(
            "You are running an MyHarness smoke test. "
            "You must use tools. "
            "1. Use write_file to create smoke.txt with the exact content MYHARNESS_E2E_OK. "
    ...
    "search_edit": Scenario(
        name="search_edit",
        prompt=(
            "You are running an MyHarness search and edit test. "
            "You must use tools. "
    ...
    "phase48": Scenario(
        name="phase48",
        prompt=(
            "You are running an MyHarness Phase 4-8 smoke test. "
            "You must use tools. "
    ...
    "task_flow": Scenario(
        name="task_flow",
        prompt=(
            "You are running an MyHarness background task test. "
            "You must use tools. "
    ...
    "skill_flow": Scenario(
        name="skill_flow",
        prompt=(
            "You are running an MyHarness skill loading test. "
            "You must use tools. "
    ...
    "mcp_model": Scenario(
        name="mcp_model",
        prompt=(
            "You are running an MyHarness MCP integration test. "
            "You must use tools. "
    ...
    "mcp_resource": Scenario(
        name="mcp_resource",
        prompt=(
            "You are running an MyHarness MCP resource test. "
            "You must use tools. "
    ...
    "agent_flow": Scenario(
        name="agent_flow",
        prompt=(
            "You are running an MyHarness agent delegation test. "
            "You must use tools. "
    ...
    "remote_agent_flow": Scenario(
        ...
        prompt=(
            "You are running an MyHarness remote-agent mode test. "
            "You must use tools. "
    ...
    "plugin_combo": Scenario(
        name="plugin_combo",
        prompt=(
            "You are running an MyHarness plugin combo test. "
            "You must use tools. "
    ...
    "ask_user_flow": Scenario(
        name="ask_user_flow",
        prompt=(
            "You are running an MyHarness ask-user test. "
            "You must use tools. "
    ...
    "task_update_flow": Scenario(
        ...
        prompt=(
            "You are running an MyHarness task update test. "
            "You must use tools. "
    ...
    "notebook_flow": Scenario(
        name="notebook_flow",
        prompt=(
            "You are running an MyHarness notebook test. "
            "You must use tools. "
    ...
    "lsp_flow": Scenario(
        name="lsp_flow",
        prompt=(
            "You are running an MyHarness LSP test. "
            "You must use tools. "
    ...
    "cron_flow": Scenario(
        name="cron_flow",
        prompt=(
            "You are running an MyHarness cron test. "
            "You must use tools. "
    ...
    "worktree_flow": Scenario(
        name="worktree_flow",
        prompt=(
            "You are running an MyHarness worktree test. "
            "You must use tools. "
    ...
    "mcp_auth_flow": Scenario(
        name="mcp_auth_flow",
        prompt=(
            "You are running an MyHarness MCP auth reconfiguration test. "
            "You must use tools. "
...
async def _run() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="myharness-e2e-suite-") as temp_dir:
        suite_root = Path(temp_dir)
        previous_env = {
            "MYHARNESS_CONFIG_DIR": os.environ.get("MYHARNESS_CONFIG_DIR"),
            "MYHARNESS_DATA_DIR": os.environ.get("MYHARNESS_DATA_DIR"),
        }
        os.environ["MYHARNESS_CONFIG_DIR"] = str(suite_root / "config")
        os.environ["MYHARNESS_DATA_DIR"] = str(suite_root / "data")
        try:
