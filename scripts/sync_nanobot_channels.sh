...
UPSTREAM_FILE="$REPO_ROOT/src/myharness/channels/UPSTREAM"
CHANNELS_DEST="$REPO_ROOT/src/myharness/channels"
...
for f in "$CHANNELS_DEST/bus/"*.py "$CHANNELS_DEST/impl/"*.py; do
  sed -i \
    -e 's/from nanobot\.bus\./from myharness.channels.bus./g' \
    -e 's/from nanobot\.channels\./from myharness.channels.impl./g' \
    -e 's/from nanobot\.config\.schema import/from myharness.config.schema import/g' \
    -e 's/from nanobot\.utils\.helpers import/from myharness.utils.helpers import/g' \
    -e 's/from nanobot\.config\.loader import/from myharness.config.loader import/g' \
    "$f"
  # Replace loguru
  sed -i \
