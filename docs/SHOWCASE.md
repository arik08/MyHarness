...
This page collects concrete ways to use MyHarness without overselling the project. Each example is intended to be small, reproducible, and easy to extend.

## 1. Repository-aware coding assistant

Use MyHarness as a lightweight local coding agent for reading code, making edits, and running validation commands.
...
MyHarness can load Markdown skills and Claude-style plugin layouts, which makes it useful for experimentation with custom workflows.

Examples:

- Put a custom skill in `~/.myharness/skills/`.
- Install a plugin into `~/.myharness/plugins/`.
...
MyHarness is useful when you need to compare Anthropic-compatible backends behind one harness.
...
- Focused on what MyHarness makes easier, not on generic LLM claims.
