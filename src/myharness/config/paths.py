"""Path resolution for MyHarness configuration and data directories.

Follows XDG-like conventions with ~/.myharness/ as the default base directory.
"""
...
def get_config_dir() -> Path:
    ...
    Resolution order:
    1. MYHARNESS_CONFIG_DIR environment variable
    2. ~/.myharness/
    """
    env_dir = os.environ.get("MYHARNESS_CONFIG_DIR")
    if env_dir:
...
def get_config_file_path() -> Path:
    """Return the path to the main settings file (~/.myharness/settings.json)."""
...
def get_data_dir() -> Path:
    ...
    Resolution order:
    1. MYHARNESS_DATA_DIR environment variable
    2. ~/.myharness/data/
    """
    env_dir = os.environ.get("MYHARNESS_DATA_DIR")
    if env_dir:
...
def get_logs_dir() -> Path:
    ...
    Resolution order:
    1. MYHARNESS_LOGS_DIR environment variable
    2. ~/.myharness/logs/
    """
    env_dir = os.environ.get("MYHARNESS_LOGS_DIR")
    if env_dir:
...
def get_project_config_dir(cwd: str | Path) -> Path:
    """Return the per-project .myharness directory."""
    project_dir = Path(cwd).resolve() / ".myharness"
