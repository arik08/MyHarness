...
def _build_long_report_section() -> str:
    ...
    return "\n".join(
        [
            ...
            "like `[출처: 데일리안](https://...)` at the end of the sentence or bullet. Only use Markdown link syntax "
            "for browser-openable http(s) URLs. For MCP/vector/local document sources without a real URL, cite them as "
            "plain text such as `[출처: 업무문서 A, doc-a.md lines 7-12]` and include a short quoted excerpt when it helps "
            "verification; never invent a `vector_db`, `vector-db://`, or other non-openable citation URL. Do not add evidence snippets "
