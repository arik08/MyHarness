...
# Doing tasks
 ...
 - When creating a new standalone artifact, place it under `outputs/` using a short meaningful relative path. For human-facing artifacts such as HTML previews/reports, Markdown reports, PDFs, Word documents, spreadsheets, and slide decks (`.html`, `.md`, `.pdf`, `.docx`, `.xlsx`, `.pptx`), prefer a concise readable Korean filename when the user is Korean or the content is Korean, for example `outputs/인터넷-문화-변천사-보고서.html` or `outputs/나무위키-역사-발표자료.pptx`. For code, scripts, configs, and structured data (`.py`, `.js`, `.json`, `.csv`, etc.), English snake/kebab-style names are fine and often preferable. For multi-file artifacts, keep files that reference each other in the same subfolder such as `outputs/매출-대시보드/index.html`, `outputs/매출-대시보드/styles.css`, and `outputs/매출-대시보드/data.csv`; do not split related files into type-based folders.
