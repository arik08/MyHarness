...
TEAMMATE_COMMAND_ENV_VAR = "MYHARNESS_TEAMMATE_COMMAND"
...
_TEAMMATE_ENV_VARS = [
    ...
    "REQUESTS_CA_BUNDLE",
    "CURL_CA_BUNDLE",
    # --- MyHarness-native provider settings --------------------------------
...
def get_teammate_command() -> str:
    ...
    Resolution order:
    1. ``MYHARNESS_TEAMMATE_COMMAND`` environment variable — allows the
       operator to point at a specific binary or wrapper script.
    2. The current Python interpreter running the ``myharness`` module.
       This keeps spawned teammates on the same venv/source tree as the
       leader process.
    3. The ``myharness`` entry-point on PATH (installed package fallback).
    """
    ...
    entry_point = shutil.which("myharness")
    if entry_point:
        return entry_point
    return "python"
...
def build_inherited_env_vars() -> dict[str, str]:
    ...
    Always includes ``MYHARNESS_AGENT_TEAMS=1`` plus any provider/proxy
