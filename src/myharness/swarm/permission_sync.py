"""Permission sync protocol for leader-worker coordination in MyHarness swarms.
...
Paths:
    ~/.myharness/teams/<teamName>/permissions/pending/<id>.json
    ~/.myharness/teams/<teamName>/permissions/resolved/<id>.json
"""
...
from myharness.swarm.lockfile import exclusive_file_lock
from myharness.swarm.mailbox import (
    MailboxMessage,
    TeammateMailbox,
...
def get_permission_dir(team_name: str) -> Path:
    """Return ~/.myharness/teams/{teamName}/permissions/"""
...
async def get_leader_name(team_name: str | None = None) -> str | None:
    ...
    """
    from myharness.swarm.team_lifecycle import read_team_file_async
...
async def handle_permission_request(
    ...
    Args:
        ...
        checker: An already-configured :class:`~myharness.permissions.checker.PermissionChecker`.
