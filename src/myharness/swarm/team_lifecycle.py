"""Persistent team lifecycle management for MyHarness swarms.

Teams are stored as JSON files on disk:
    ~/.myharness/teams/<name>/team.json
...
async def _kill_orphaned_teammate_panes(team_name: str) -> None:
    ...
    from myharness.swarm.spawn_utils import is_inside_tmux
    from myharness.swarm.types import is_pane_backend
...
class TeamLifecycleManager:
    ...
    Persists team metadata to ``~/.myharness/teams/<name>/team.json``.
    ...
    def list_teams(self) -> list[TeamFile]:
        """Return all teams found in ``~/.myharness/teams/``, sorted by name."""
        base = Path.home() / ".myharness" / "teams"
        if not base.exists():
            return []
