...
class RepoAutopilotStore:
        ...
        for label, path in discovered[:limit]:
            ...
            card, _ = self.enqueue_card(
                ...
                body=(
                    ...
                    "Review whether this should be aligned, adapted, or ignored for MyHarness."
                ),
    ...
    def _comment_started(self, card: RepoTaskCard, attempt_count: int) -> str:
        return _bilingual_lines(
            f"MyHarness autopilot 已开始处理 `{card.id}`，当前第 {attempt_count} 轮执行。",
            f"MyHarness autopilot started processing `{card.id}`. Attempt {attempt_count} is now running.",
        )
    ...
    def _render_dashboard_html(self, snapshot: dict[str, Any]) -> str:
        ...
        """
        repo_name = escape(_safe_text(snapshot.get("repo_name")) or "MyHarness")
