...
from myharness.api.codex_client import _build_codex_headers, _resolve_codex_url
from myharness.auth.external import CODEX_PROVIDER, default_binding_for_provider, load_external_credential
...
async def _generate_with_images_api(
    ...
    *,
    ...
    default_headers: dict[str, str] | None = None,
        ...
        resolved_base_url = (base_url or os.environ.get("MYHARNESS_IMAGE_BASE_URL", "")).strip()
...
async def _generate_with_codex_oauth(
    ...
    url = _resolve_codex_url(os.environ.get("MYHARNESS_CODEX_BASE_URL"))
...
def _resolve_api_key() -> str:
    return (
        os.environ.get("MYHARNESS_IMAGE_API_KEY", "").strip()
...
def _resolve_codex_request_model(context: ToolExecutionContext) -> str:
    configured = os.environ.get("MYHARNESS_CODEX_IMAGE_MODEL", "").strip()
    if configured:
        return configured
