"""Comprehensive integration tests for all previously untested MyHarness features.
...
async def test_hooks_in_agent_loop():
    ...
    from myharness.api.client import AnthropicApiClient
    from myharness.config.settings import PermissionSettings
    from myharness.engine.query import QueryContext, run_query
    from myharness.engine.messages import ConversationMessage
    from myharness.engine.stream_events import AssistantTextDelta, ToolExecutionStarted, ToolExecutionCompleted
    from myharness.permissions.checker import PermissionChecker
    from myharness.permissions.modes import PermissionMode
    from myharness.tools.base import ToolRegistry
    from myharness.tools.bash_tool import BashTool
...
async def test_skills_load():
    ...
    from myharness.skills.loader import load_user_skills
...
async def test_memory_lifecycle():
    ...
    from myharness.memory.manager import list_memory_files, add_memory_entry, remove_memory_entry
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        mem_dir = Path(tmpdir) / ".myharness" / "memory"
        ...
        p2 = add_memory_entry(tmpdir, "project-goal", "Building an AI agent framework called MyHarness")
...
async def test_commands_registry():
    ...
    # Default registry
    from myharness.commands.registry import create_default_command_registry
...
async def test_mcp_types():
    ...
    from myharness.mcp.types import McpStdioServerConfig, McpToolInfo, McpConnectionStatus
...
async def test_config_paths():
    ...
    # All should be under ~/.myharness
    all_under_home = all(".myharness" in str(p) for p in paths.values())
...
async def test_combined_hooks_skills_agent():
    ...
    from myharness.engine.query import QueryContext, run_query
    from myharness.engine.messages import ConversationMessage
    from myharness.engine.stream_events import AssistantTextDelta, ToolExecutionStarted
    from myharness.permissions.checker import PermissionChecker
    from myharness.permissions.modes import PermissionMode
    from myharness.tools.base import ToolRegistry
    from myharness.tools.bash_tool import BashTool
...
async def test_full_swarm_autoagent():
    ...
    from myharness.tools.glob_tool import GlobTool
    from myharness.tools.grep_tool import GrepTool
    from myharness.swarm.team_lifecycle import TeamLifecycleManager, TeamMember
    import myharness.swarm.mailbox as mb
    import myharness.swarm.team_lifecycle as tl
