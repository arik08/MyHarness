...
def test_build_system_prompt_plans_substantial_tasks_first():
    ...
    assert "At a meaningful transition into writing or editing, emit a short progress marker, then call the file tool immediately." in prompt
    assert "stream the actual tool arguments so users can inspect the execution" in prompt
