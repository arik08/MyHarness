...
class TestTokenLimitParams:
    ...
    def test_gpt54_mini_uses_max_completion_tokens(self):
        assert _token_limit_param_for_model("gpt-5.4-mini", 42000) == {"max_completion_tokens": 42000}
...
class TestStreamMessageTokenParams:
    ...
    async def test_gpt5_stream_uses_max_completion_tokens(self):
        ...
        assert events
        assert fake_sdk.chat.completions.last_kwargs is not None
        assert fake_sdk.chat.completions.last_kwargs["max_completion_tokens"] == 42000
    ...
    @pytest.mark.asyncio
    async def test_gpt54_mini_stream_uses_configured_completion_token_cap(self):
        client = OpenAICompatibleClient(api_key="test-key")
        fake_sdk = _FakeOpenAIClient()
        client._client = fake_sdk
