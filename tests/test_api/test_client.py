...
from myharness.engine.messages import ConversationMessage, ImageBlock, TextBlock
from myharness.api.client import AnthropicApiClient, OAUTH_BETA_HEADER
...
def test_anthropic_client_adds_claude_oauth_identity_headers(monkeypatch):
    ...
    monkeypatch.setattr("myharness.api.client.AsyncAnthropic", _FakeAsyncAnthropic)
    monkeypatch.setattr(
        "myharness.auth.external.get_claude_code_version",
        lambda: "2.1.92",
    )
    monkeypatch.setattr(
        "myharness.auth.external.get_claude_code_session_id",
        lambda: "session-123",
    )
    monkeypatch.setattr(
        "myharness.api.client.get_claude_code_session_id",
        lambda: "session-123",
    )
    monkeypatch.setattr(
        "myharness.api.client.claude_attribution_header",
