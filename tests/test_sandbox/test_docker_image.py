...
from myharness.sandbox.docker_image import (
    _image_exists,
...
async def test_image_exists_returns_true_on_success(monkeypatch):
    monkeypatch.setattr(
        "myharness.sandbox.docker_image.shutil.which",
    ...
    result = await _image_exists("myharness-sandbox:latest")
...
async def test_ensure_image_skips_build_when_exists(monkeypatch):
    ...
    result = await ensure_image_available("myharness-sandbox:latest", auto_build=True)
    assert result is True
