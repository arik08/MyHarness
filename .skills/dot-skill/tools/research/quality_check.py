...
def evaluate_skill_text(
    ...
    checks = {
        "mental_models": bullet_count >= 3 and bool(re.search(r"mental model|decision framework|operating model", text, re.IGNORECASE)),
        "limitations": bool(re.search(r"limitations|boundary|known limits", text, re.IGNORECASE)),
        "expression_dna": bool(re.search(r"expression DNA|sentence rhythm|metaphor", text, re.IGNORECASE)),
        "honest_boundaries": bool(re.search(r"honest boundar|what .* does not know|evidence boundaries", text, re.IGNORECASE)),
        "internal_tension": bool(re.search(r"contradiction|tension|tradeoff", text, re.IGNORECASE)),
