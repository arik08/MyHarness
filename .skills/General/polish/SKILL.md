---
name: polish
description: "Use when doing a final MyHarness UI polish and QA pass: design-system alignment, spacing, density, text clipping, overlap, responsive behavior, interaction states, accessibility, console errors, and small frontend refinements before shipping."
---

# Polish

Use this skill for final UI/UX cleanup after a feature is functionally complete. In MyHarness, polish means a slim, dense, work-focused office UI: avoid oversized padding, over-rounded controls, decorative marketing treatment, generic AI aesthetics, and visible text explaining the UI itself.

Always align with the project instructions in `AGENTS.md`. After creating or changing UI, inspect the actual screen with the Codex app browser controls when possible; fall back to Playwright only if the Codex app browser is unavailable. Verify that major screens are not broken, text does not overlap or clip, and the console has no relevant errors.
