...
1. Inspect the catalog before guessing site or operation names.
   ```powershell
   python .skills/General/openweb/scripts/openweb_call.py sites
   python .skills/General/openweb/scripts/openweb_call.py youtube
   ```
...
3. Inspect operation parameters before calling it.
   ```powershell
   python .skills/General/openweb/scripts/openweb_call.py arxiv searchPapers
   python .skills/General/openweb/scripts/openweb_call.py wikipedia getPageSummary
   ```
4. Call the operation through the wrapper using `key=value` parameters. This
   avoids PowerShell JSON quote corruption.
   ```powershell
   python .skills/General/openweb/scripts/openweb_call.py arxiv searchPapers "search_query=all:agent web api" max_results=3
   python .skills/General/openweb/scripts/openweb_call.py wikipedia getPageSummary title=World_Wide_Web
   ```
