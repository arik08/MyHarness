...
Apply this skill in any environment that can create HTML artifacts. Check the available preview, file-writing, rendering, and capture capabilities rather than assuming a particular application supplies them.
...
- For single-file previews, keep CSS and app code in the HTML file when practical so the viewer can open it directly; relative external files can break in `iframe srcdoc` previews.
...
- **A4 landscape page report**: only when the user explicitly asks for A4 landscape, A4 가로, 가로형 A4, printable horizontal PDF, or a fixed-page report, use this skill for HTML/browser visual direction. Use a suitable fixed-page report skill if available (for example, `html-a4-landscape-report`); otherwise implement explicit page dimensions, density limits, table splitting, and overflow QA directly. For A4 PPTX, use available presentation-generation tools with the same page-layout checks.
...
- Include the CSS and any JS needed for `.source-ref` badges and hover/focus tooltips in the HTML itself. If the execution environment explicitly provides equivalent assets, use its documented integration and verify the saved file; do not assume a proprietary marker or save-time injection is available. Keep tooltip text readable, keyboard-accessible, and within the viewport, and avoid duplicate tooltip handlers.
...
- Populate source tooltips from retrieved source evidence. Use the short source address on the first line and a short verbatim excerpt from the source/tool result on the second line, wrapped in double quotes. Escape text when inserting it into HTML attributes such as `data-tooltip`. If the environment explicitly supplies evidence-backed tooltip enrichment, let it populate the excerpt and verify the saved result. Otherwise include the verified excerpt yourself; never invent a quotation or assume an unavailable tool will fill it later. If no verified excerpt is available, keep the source link and disclose the evidence limitation without fabricating text.
...
- For HTML diagrams, include Mermaid.js and initialize it in the artifact itself unless the target viewer explicitly provides a compatible renderer. Do not assume chat or Markdown rendering also applies inside an HTML preview. Verify the saved artifact in its intended viewer, and do not leave visible raw Mermaid text or placeholder boxes in the finished preview.
...
- **Mermaid**: maintainable diagrams in the Mermaid.js family, not only flowcharts. Prefer Mermaid over hand-drawn SVG for structured diagrams when the visual type fits, because Mermaid is usually cleaner and easier to revise. In chat or Markdown viewers that support Mermaid, prefer fenced `mermaid` blocks for compact diagrams such as `flowchart`, `sequenceDiagram`, `classDiagram`, `stateDiagram`, `erDiagram`, `gantt`, `pie`, `journey`, `gitGraph`, `timeline`, `mindmap`, `quadrantChart`, and `sankey`. Choose the type that fits the content instead of defaulting to flowchart. Use `quadrantChart` freely for 2x2 prioritization/positioning and use `sankey` for flow/allocation diagrams; validate Mermaid with an available parser or renderer before delivery so syntax errors can be fixed before preview; do not assume the file-writing tool performs validation. Avoid Mermaid keyword-like flowchart class names such as `end`; use names like `finish` or `done`. If a requirement diagram is the right choice, quote user-defined `id`/`text` values and use canonical enum casing so the Mermaid renderer can render it reliably. In standalone HTML artifacts, include Mermaid via CDN only when the artifact needs those diagram types, and render from `<div class="mermaid">...</div>` blocks rather than showing Mermaid as code.
...
- Use actual ECharts initialization and data series, not just a script import or a chart-shaped placeholder. Load the library explicitly in standalone HTML; do not assume the parent application page exposes it inside the preview.
...
5. If the user wants screenshots/PDF, capture the rendered HTML with available browser/export tools; use `playwright-capture` if available. Verify the exported result and disclose any unavailable capture capability.
6. For important or dense visuals, inspect clipping, overflow, chart labels, and print layout in the rendered result; use `visual-review` if available. Disclose any runtime verification gap rather than treating source inspection as visual verification.
...
- For A4 landscape HTML/PDF/browser reports, keep visual direction here and apply a fixed-page layout workflow, using a suitable report skill if available. Check page dimensions, density, table splits, and overflow. For A4 landscape PPTX, apply the same checks using available presentation-generation tools.
